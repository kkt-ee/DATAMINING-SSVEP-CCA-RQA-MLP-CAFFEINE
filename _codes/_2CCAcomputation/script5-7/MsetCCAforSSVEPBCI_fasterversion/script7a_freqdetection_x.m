...
load('/mnt/pd/vars/SSVEP_8ch_A1-6.mat') % <<--------enter from script 6, further aalysis must be done for both non caffeinated and caffeinated data
X=newsig;
...
%% [X] -c-cannonical variate AND CCA(X,Y) :: CCA for SSVEP recognition
%function coeffs = computecca(X4D,Y)
sigDetect = zeros(7,6); % ch, ph, v
%for k=1:8%22 % => 4D data of 22Channels                                                               % Ch+Neighbour (5 x 5121 x 3 x 7) x 20 --pick one and process
    %fprintf('[ch=%d] ',k)
        ...
        for tw_length=1:1:t_length %:4 *      % time window length:  1s:1s:4s                            % 1:timeWindow(in sec)
            ...
            for j=1:7 %:4 *                                                                             % 7Photic stimulus
                ...
                if idx==j                                                  % it means frequency match detected
                    ...
                    sigDetect(j,run)=1;                                   % <new entry>
                    hits(j,run)=hits(j,run)+1;                          % <new entry><-----OUTPUT
...
%for chx = 1:8%22
    for px = 1:7
        % ---------------deduction #1       
        tempvec = hits(px,:);
        sumDetectallVol1D(px) = sum(tempvec); clearvars tempvec   %---#1
...
%Channels={'T5' 'O1' 'PZ' 'OZ' 'T4' 'T6' 'P4' 'O2'};
%c=categorical(Channels);

%-- barplot 1--
%figure, bar(sumDetectallVol1D)%(:,2:end))
    %clearvars A normA
    
    %normA=(normc(sumDetectallVol1D'));
    
    A=sumDetectallVol1D;%=featureScaling(hits)
    %normA2 = A - min(A(:))
    normA2 = A;%A - min(A(:))
    normA2 = normA2 ./ max(normA2(:))
    ...
    xlabel('Stimulus frequency','FontSize', 25), ylabel('SSVEP hit','FontSize', 25), title(figheader); 
    ylim([0,1])
    ...
    %% save plot  and the variable norm A2.
