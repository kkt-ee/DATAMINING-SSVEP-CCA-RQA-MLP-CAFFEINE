function [net,tr]=newrb1(p,t,goal,spread,mn,df) 
%newrb1 Design a radial basis network. 
% 
%  Synopsis 
% 
%    net = newrb1 
%    [net,tr] = newrb1(P,T,GOAL,SPREAD,MN,DF) 
% 
%  Description 
% 
%    Radial basis networks can be used to approximate 
%    functions.  newrb1 adds neurons to the hidden 
%    layer of a radial basis network until it meets 
%    the specified mean squared error goal. 
% 
%   NET = newrb1 creates a new network with a dialog box. 
% 
%   newrb1(PR,T,GOAL,SPREAD,MN,DF) takes these arguments, 
...
%   and the network may not generalize well.  Call newrb1 with 
...
%  Algorithm 
% 
%    newrb1 creates a two layer network. The first layer has RADBAS 
...
% 
%  See also SIM, newrb1E, NEWGRNN, NEWPNN. 
 
% Mark Beale, 11-31-97 
% Copyright 1992-2002 The MathWorks, Inc. 
% $Revision: 1.9 $ $Date: 2002/04/14 21:36:23 $ 
...
tr.perf(2) = sse; 
if isfinite(df) 
  fprintf('newrb1, neurons = 0, SSE = %g\n',sse); 
end 
flag_stop = 0; 
...
for k = 2:mn 
  ...
  if isfinite(df) & (~rem(k,df)) 
    fprintf('newrb1, neurons = %g, SSE = %g\n',k,sse); 
    flag_stop=plotperf(tr,eg,'newrb1',k); 
  end 
