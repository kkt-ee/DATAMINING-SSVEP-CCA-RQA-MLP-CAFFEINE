...
function [PerfTrain, PerfTest, PerfValidate, PerfTotal] = nnmlp(xlsxfile,impPredictors,a,b,NNparam,Neurons)
%% testing
%{
xlsxfile
sheetname
...
[num,txt,~] = xlsread(xlsxfile);              % read excel data file
...
%%
features = {'recrate', 'DET', 'LMAX', 'ENT', 'LAM', 'TT', 'VMAX', 'RATIO', 'AN', 'DIV', 'AD'};  %all features
...
%(remove %) nntraintool                                 % open NN GUI
%(remove %) plotperform(tr)                             % performance graph
...
allX = nn_input; allT = nn_Target; allY = net(allX);
