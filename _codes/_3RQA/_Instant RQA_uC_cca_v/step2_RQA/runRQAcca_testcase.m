...
load /mnt/pd/vars/SSVEPdataA1-6.mat  %---------------enter
label='A'; % <<------------------------------------------enter
fs=256;
    ...
    for px = 1%:7                % 5 hz to 30 hz signals ( 6 Nos)
        ...
        for vx = 1%:6            % 6 volunteers           
            ...
            for segmentno=1%:20  % 20 segments of 1second
                ...
                b=a+span;
                sigsegment = SSVEPdata(chx,:,vx,px); % sig = SSVEPdata(chx,a:b,vx,px); <<----------enter
                %%
                filtsigsegment = filtfilt(bandFilt,sigsegment);  % applying bandpass filter
                RQAstat(segmentno,:) = RQA(filtsigsegment);      % calling main()     % savefilename (change)           
        ...
        filename=strcat(resultdir,label,chxhead,pxhead,'bfall6vol.csv');
